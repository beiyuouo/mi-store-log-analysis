...
# 百度 ak 自行申请 http://lbsyun.baidu.com/index.php?title=webapi/ip-api
AK = "ODS3wQ91B86fFeyk3k0UnsowPRrCToYi"
...
class NginxLog:
    ...
    def __init__(self, *args):
        ...
        self.device, self.os, self.browser = str(
            parse(http_user_agent)).split(' / ')
    ...
    def save(self):
        if not re.findall('\.[css|js|woff|TTF|png|jpg|ico]', log.request):
            ...
            df = pd.DataFrame([value])
            print(value)
            df.to_sql('fact_nginx_log', con=ENGINE,
